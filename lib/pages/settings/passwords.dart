...
class PasswordSelectionScreen extends StatefulWidget {
  ...
  const PasswordSelectionScreen({required this.email, Key? key})
      : super(key: key);

  @override
  _PasswordSelectionScreenState createState() =>
      _PasswordSelectionScreenState();
}
...
class _PasswordSelectionScreenState extends State<PasswordSelectionScreen> {
  ...
  void _sendPasswords() {
    if (_selectedPasswords.isNotEmpty) {
      ...
      print(
          'Sending passwords: ${_selectedPasswords.join(', ')} to ${widget.email}');
    } else {
